...
import { TokenUnfreezeTransaction, TransactionReceipt } from "@hashgraph/sdk";

const TokenUnfreezeAccountModal: FC<IModalProps> = ({ setLoadModal }) => {
        ...
        <Modal width="sm:max-w-2xl" setShow={setLoadModal}>
            <article className="px-4 py-2 w-80">
                <div className="border-b border-gray-200 bg-white px-4 py-4 sm:px-6 text-center">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Unfreeze Token</h3>
                </div>

                <section className="py-4">
                    <label htmlFor="accountId" className="block text-sm font-medium text-gray-700">
                        Account to sign (token freeze key account)
                    </label>
                    <select
                        id="accountId"
...
export default TokenUnfreezeAccountModal;