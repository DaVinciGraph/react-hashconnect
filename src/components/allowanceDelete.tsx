...
const AllowanceDeleteModal: FC<IModalProps> = ({ setLoadModal }) => {
    ...
    const send = async () => {
        try {
            if (!tokenId)
                throw new Error("Enter the token ID.");

            let trans = new AccountAllowanceDeleteTransaction()
                .deleteAllTokenNftAllowances(new NftId(TokenId.fromString(tokenId), 1), signingAcct);
        ...
        <Modal width="sm:max-w-2xl" setShow={setLoadModal}>
            <article className="py-6 px-4">
                <div className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6 text-center">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Allowance Delete</h3>
                </div>
...
export default AllowanceDeleteModal;