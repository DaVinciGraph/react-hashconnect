...
import { TokenWipeTransaction, TransactionReceipt } from "@hashgraph/sdk";

const WipeTokenModal: FC<IModalProps> = ({ setLoadModal }) => {
        ...
        <Modal width="sm:max-w-2xl" setShow={setLoadModal}>
            <article className="px-4 py-2 w-80">
                <div className="border-b border-gray-200 bg-white px-4 py-4 sm:px-6 text-center">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Wipe token</h3>
                </div>

                <section className="py-4">
                    <label htmlFor="accountId" className="block text-sm font-medium text-gray-700">
                        Account to sign (token wipe account)
                    </label>
                    <select
                        id="accountId"
                ...
                {isNft && <section className="py-2">
                    ...
                    {serials.map((serial, index) => <div className="mt-1" key={serial}>
                        <input
                            className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            type="text" name="" value={serial} onChange={(event: any) => { changeSerial(index, event.target.value) }} />
                    </div>)}
