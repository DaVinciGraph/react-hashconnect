...
import { ContractCreateTransaction, ContractFunctionParameters, TransactionReceipt } from "@hashgraph/sdk";

const SmartContractCreateModal: FC<IModalProps> = ({ setLoadModal }) => {
    ...
    const [bytecodeFileId, setBytecodeFileId] = useState('0.0.30861882');
        ...
        <Modal width="sm:max-w-2xl" setShow={setLoadModal}>
            <article className="px-4 py-2 w-80">
                <div className="border-b border-gray-200 bg-white px-4 py-4 sm:px-6 text-center">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Smart Contract Create</h3>
                </div>
                ...
                <section className="py-2">
                    <label htmlFor="byteCodeId" className="block text-sm font-medium text-gray-700">
                        Bytecode File Id:
                    </label>
                    <div className="mt-1">
                        <input
                            className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            type="text" name="" id="byteCodeId" value={bytecodeFileId} onChange={(event: any) => { setBytecodeFileId(event.target.value) }} />
                    </div>
                </section>
...
export default SmartContractCreateModal;