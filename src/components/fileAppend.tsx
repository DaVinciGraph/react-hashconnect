...
import { FileAppendTransaction, TransactionReceipt } from "@hashgraph/sdk";

const FileAppendModal: FC<IModalProps> = ({ setLoadModal }) => {
    ...
    const send = async () => {
        ...
        let trans = new FileAppendTransaction()
            .setFileId(fileId)
        ...
        <Modal width="sm:max-w-2xl" setShow={setLoadModal}>
            <article className="px-4 py-2 w-80">
                <div className="border-b border-gray-200 bg-white px-4 py-4 sm:px-6 text-center">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">File Append</h3>
                </div>

                <p>The default file content is the contract code from the <a href="https://hedera.com/blog/how-to-deploy-smart-contracts-on-hedera-part-1-a-simple-getter-and-setter-contract" rel="noreferrer" className="underline" target="_blank">Hedera smart contract tutorial</a></p>
                ...
                <section className="py-2">
                    ...
                    <div className="mt-1">
                        <input
                            ...
                            type="text" name="" id="fileId" value={fileId} onChange={(event: any) => { setFileId(event.target.value) }} />
                    </div>
                </section>
...
export default FileAppendModal;